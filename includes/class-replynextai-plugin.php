...
final class ReplyNextAI_Plugin {
    ...
    public function sanitize_options($input) {
        ...
        add_settings_error('replynextai_messages', 'replynextai_saved', __('ReplyNext settings saved.', 'replynextai-chat'), 'updated');
        return $output;
    }
    ...
    public function admin_menu() {
        add_menu_page(__('ReplyNext AI', 'replynextai-chat'), __('ReplyNext AI', 'replynextai-chat'), 'manage_options', 'replynextai', array($this, 'dashboard_page'), 'dashicons-format-chat', 58);
        add_submenu_page('replynextai', __('Dashboard', 'replynextai-chat'), __('Dashboard', 'replynextai-chat'), 'manage_options', 'replynextai', array($this, 'dashboard_page'));
        add_submenu_page('replynextai', __('AI Website Chat', 'replynextai-chat'), __('AI Website Chat', 'replynextai-chat'), 'manage_options', 'replynextai-ai', array($this, 'ai_page'));
        add_submenu_page('replynextai', __('Free WhatsApp', 'replynextai-chat'), __('Free WhatsApp', 'replynextai-chat'), 'manage_options', 'replynextai-whatsapp', array($this, 'whatsapp_page'));
        add_submenu_page('replynextai', __('Display Rules', 'replynextai-chat'), __('Display Rules', 'replynextai-chat'), 'manage_options', 'replynextai-display', array($this, 'display_page'));
        add_submenu_page('replynextai', __('Connection', 'replynextai-chat'), __('Connection', 'replynextai-chat'), 'manage_options', 'replynextai-connection', array($this, 'connection_page'));
        add_submenu_page('replynextai', __('WooCommerce', 'replynextai-chat'), __('WooCommerce', 'replynextai-chat'), 'manage_options', 'replynextai-woocommerce', array($this, 'woocommerce_page'));
        add_submenu_page('replynextai', __('Help', 'replynextai-chat'), __('Help & Shortcodes', 'replynextai-chat'), 'manage_options', 'replynextai-help', array($this, 'help_page'));
    }

    public function plugin_links($links) {
        array_unshift($links, '<a href="' . esc_url(admin_url('admin.php?page=replynextai')) . '">' . esc_html__('Dashboard', 'replynextai-chat') . '</a>', '<a href="' . esc_url(admin_url('admin.php?page=replynextai-ai')) . '">' . esc_html__('Settings', 'replynextai-chat') . '</a>');
        return $links;
    }
    ...
    private function page_header($title, $description) {
        ?>
        <div class="rn-admin-header"><div><span class="rn-eyebrow"><?php esc_html_e('ReplyNext AI for WordPress', 'replynextai-chat'); ?></span><h1><?php echo esc_html($title); ?></h1><p><?php echo esc_html($description); ?></p></div><a class="button button-secondary" href="<?php echo esc_url($this->portal_url()); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e('Open Client Portal ↗', 'replynextai-chat'); ?></a></div>
        <?php
    }
    ...
    public function dashboard_page() {
        ...
        ?>
        <div class="wrap rn-wrap"><?php $this->page_header(__('Dashboard', 'replynextai-chat'), __('Launch free WhatsApp chat or connect your ReplyNext AI sales assistant.', 'replynextai-chat')); ?>
        <div class="rn-grid rn-grid-3"><?php $this->status_card(__('AI Website Chat', 'replynextai-chat'), $ai_ready, $ai_ready ? __('Live and connected', 'replynextai-chat') : __('Company ID and activation required', 'replynextai-chat'), 'replynextai-ai'); ?><?php $this->status_card(__('Free WhatsApp', 'replynextai-chat'), $wa_ready, $wa_ready ? __('Live using your own number', 'replynextai-chat') : __('Add a WhatsApp number', 'replynextai-chat'), 'replynextai-whatsapp'); ?><?php $this->status_card(__('Display Rules', 'replynextai-chat'), true, __('Control pages and devices', 'replynextai-chat'), 'replynextai-display'); ?></div>
        <div class="rn-card rn-setup-card"><div><span class="rn-step"><?php esc_html_e('QUICK START', 'replynextai-chat'); ?></span><h2><?php esc_html_e('Start converting visitors in three steps', 'replynextai-chat'); ?></h2></div><ol class="rn-checklist"><li class="<?php echo $o['server_url'] ? 'done' : ''; ?>"><span>1</span><?php esc_html_e('Confirm your ReplyNext server URL', 'replynextai-chat'); ?></li><li class="<?php echo ($ai_ready || $wa_ready) ? 'done' : ''; ?>"><span>2</span><?php esc_html_e('Enable AI Chat or Free WhatsApp', 'replynextai-chat'); ?></li><li><span>3</span><?php esc_html_e('Open your website privately and send a test message', 'replynextai-chat'); ?></li></ol></div>
        <div class="rn-grid rn-grid-2"><div class="rn-card"><h2><?php esc_html_e('Free WhatsApp, no API required', 'replynextai-chat'); ?></h2><p><?php esc_html_e('Open a direct conversation using your own WhatsApp number.', 'replynextai-chat'); ?></p><a class="button button-primary" href="<?php echo esc_url(admin_url('admin.php?page=replynextai-whatsapp')); ?>"><?php esc_html_e('Configure Free WhatsApp', 'replynextai-chat'); ?></a></div><div class="rn-card rn-upgrade"><h2><?php esc_html_e('Turn chat into an AI sales agent', 'replynextai-chat'); ?></h2><p><?php esc_html_e('Capture leads and manage AI conversations in ReplyNext.', 'replynextai-chat'); ?></p><a class="button button-secondary" href="<?php echo esc_url(admin_url('admin.php?page=replynextai-ai')); ?>"><?php esc_html_e('Connect AI Website Chat', 'replynextai-chat'); ?></a></div></div></div>
        <?php
    }

    private function status_card($title, $ready, $message, $page) {
        ?><a class="rn-card rn-status-card" href="<?php echo esc_url(admin_url('admin.php?page=' . $page)); ?>"><span class="rn-status <?php echo $ready ? 'is-live' : 'is-off'; ?>"><?php echo $ready ? esc_html__('Active', 'replynextai-chat') : esc_html__('Setup', 'replynextai-chat'); ?></span><h2><?php echo esc_html($title); ?></h2><p><?php echo esc_html($message); ?></p></a><?php
    }
    ...
    public function ai_page() {
        $o = $this->options();
        ?><div class="wrap rn-wrap"><?php $this->page_header(__('AI Website Chat', 'replynextai-chat'), __('Connect the AI chat module configured in your ReplyNext client portal.', 'replynextai-chat')); ?><?php $this->form_open(); $this->preserve_fields(array('server_url', 'company_id', 'ai_enabled', 'ai_sitewide', 'share_user_data')); ?><div class="rn-grid rn-grid-form"><div class="rn-card">
    ...
    public function handle_test_connection() {
        if (!current_user_can('manage_options')) wp_die(esc_html__('Unauthorized', 'replynextai-chat'));
    ...
    public function woocommerce_page() {
        ...
        ?><div class="wrap rn-wrap"><?php $this->page_header(__('WooCommerce', 'replynextai-chat'), __('Sync products into the ReplyNext AI catalog for accurate recommendations.', 'replynextai-chat')); ?><div class="rn-grid rn-grid-form"><div class="rn-card"><span class="rn-status <?php echo $available ? 'is-live' : 'is-off'; ?>"><?php echo $available ? esc_html__('Detected', 'replynextai-chat') : esc_html__('Unavailable', 'replynextai-chat'); ?></span><h2><?php esc_html_e('Product catalog sync', 'replynextai-chat'); ?></h2><p><?php esc_html_e('Syncs up to 100 published WooCommerce products with names, prices, descriptions, categories, links, and images.', 'replynextai-chat'); ?></p><?php if ($status) : ?><p><strong><?php echo esc_html($status); ?></strong></p><?php endif; ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="replynextai_sync_products" /><?php wp_nonce_field('replynextai_sync_products'); ?><?php submit_button(__('Sync Products Now', 'replynextai-chat'), 'primary', 'submit', false, $available ? array() : array('disabled' => 'disabled')); ?></form></div><div class="rn-card"><h2><?php esc_html_e('Automatic updates', 'replynextai-chat'); ?></h2><p><?php esc_html_e('After a successful connection, product saves schedule a background catalog refresh.', 'replynextai-chat'); ?></p><p><?php esc_html_e('Requirements: WooCommerce active and a valid ReplyNext WordPress token.', 'replynextai-chat'); ?></p></div></div></div><?php
    }
    ...
    public function help_page() {
        ?><div class="wrap rn-wrap"><?php $this->page_header(__('Help & Shortcodes', 'replynextai-chat'), __('Use automatic display, shortcodes, or both.', 'replynextai-chat')); ?><div class="rn-grid rn-grid-2"><div class="rn-card"><h2><?php esc_html_e('AI Website Chat', 'replynextai-chat'); ?></h2><p><code>[replynextai_chat]</code></p><p><code>[replynextai_chat company_id="12" server_url="https://replynextai.com"]</code></p></div><div class="rn-card"><h2><?php esc_html_e('Free WhatsApp', 'replynextai-chat'); ?></h2><p><code>[replynextai_whatsapp]</code></p><p><code>[replynextai_whatsapp number="8801XXXXXXXXX" message="Hello"]</code></p></div></div></div><?php
    }
    ...
    public function ai_shortcode($attributes) {
        ...
        if (!$this->enqueue_ai($config) && current_user_can('manage_options')) return '<p class="replynextai-chat-error">' . esc_html__('ReplyNext AI Chat is not configured.', 'replynextai-chat') . '</p>';
